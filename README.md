...
|---|---|---|---|
| Java / JVM | [candor-java](https://github.com/tombaldwin/candor-java) | ASM bytecode + CHA | **shipped (spec 0.32)** — the **reference engine**; full mode set incl. `--gate-json`; Spring-aware; Java/Kotlin/Scala/Groovy (`jbang candor@tombaldwin/candor-java`) |
| Rust | [candor-rust](https://github.com/tombaldwin/candor-rust) | dylint (HIR + CHA) + a stable `syn` scanner | **shipped (spec 0.32)** — `cargo install candor-scan` |
| TypeScript | [candor-ts](https://github.com/tombaldwin/candor-ts) | TS compiler API | **shipped (spec 0.32)** — project scanning, gate, queries, MCP + LSP servers; on npm (`npx -y candor-ts`) |
| Swift | [candor-swift](https://github.com/tombaldwin/candor-swift) | SwiftParser (syntactic) | **shipped (spec 0.32)** — receiver-typed local inference; the 4th conformance engine |
...
|---|---|---|---|
| Agent fleets | [candor-agents](https://github.com/tombaldwin/candor-agents) | declared-vs-observed effect analysis over agent fleets (units = agents/sessions/hooks) | **shipped (spec 0.32)** — v0.8.1; `pipx install git+https://github.com/tombaldwin/candor-agents` |
