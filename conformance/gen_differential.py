...
# =====================================================================================================
# ======================================================================================================
# THE SHARED FIXTURE-TREE WRITERS — one owner for "what a scannable <language> tree looks like".
#
# SOUNDNESS R288's shape, found by a 2026-09-12 audit and landed 2026-09-13: `_rust_tree`, `_java_tree`
# and `_ts_tree` each existed in THREE generators (`gen_masking`, `gen_netclass`, `gen_policy_match`) in
# THREE variants apiece — nine near-copies of "write a Cargo.toml and a lib.rs". The variance was NOT rot;
# every difference was real parameterisation (the crate/package name, the `GENERATED by` header, which
# imports a TypeScript fixture needs, and where the source file lands). That is exactly what makes it
# worth extracting rather than deduplicating: the copies agreed until they needed to differ, and an
# instrument with three copies has no owner, so a defect found in one is not fixed in the other two.
#
# WHAT IS AND IS NOT PARAMETERISED, stated because the next person will be tempted to add a hook:
# the BODY arrives already `.format()`-ed and already wrapped in whatever the caller needs (a
# `pub mod <scope>`, a package), and post-write work (`_write_config`) stays at the CALL SITE. These
# writers do one thing — lay down a minimal tree a scanner will accept — so there is no callback, no
# `case` dict, and no branch on which generator is calling. A writer that knew its callers apart would
# be the three copies again with an `if` in front.
#
# A `gen` argument rather than deriving the caller's filename by frame inspection: the header is a
# statement about which generator OWNS the fixture, and deriving it would make a file moved between
# generators quietly claim the wrong owner.


def write_rust_tree(d, name, gen, body):
    """A single-crate rust package: Cargo.toml + src/lib.rs carrying `body`."""
    os.makedirs(os.path.join(d, "src"), exist_ok=True)
    with open(os.path.join(d, "Cargo.toml"), "w") as f:
        f.write(f'[package]\nname = "{name}"\nversion = "0.0.0"\nedition = "2021"\n')
    with open(os.path.join(d, "src", "lib.rs"), "w") as f:
        f.write(f"// GENERATED by {gen} -- do not edit.\n" + body + "\n")


def write_java_tree(d, gen, body, pkg="q", src=None):
    """A one-class java package: <pkg>/E.java, `body` as the class's members.

    `src` overrides the destination for generators whose package path is case-derived; it must already
    sit under `d`. The MEMBERS-ONLY contract is the caller's to honour — this wraps in `package`/`class`
    and a body carrying its own class declaration will not compile, loudly, at scan time."""
    os.makedirs(os.path.join(d, *pkg.split(".")), exist_ok=True)
    dest = src if src is not None else os.path.join(d, pkg, "E.java")
    with open(dest, "w") as f:
        f.write(f"// GENERATED by {gen} -- do not edit.\n")
        f.write(f"package {pkg};\npublic class E {{\n" + body + "\n}\n")


TS_STD_IMPORTS = ('import * as fsm from "node:fs";\n'
                  'import * as netm from "node:net";\n'
                  'import * as cp from "node:child_process";\n'
                  'import { DatabaseSync } from "node:sqlite";\n')


def write_ts_tree(d, name, gen, body, imports=TS_STD_IMPORTS, sub=None):
    """A minimal npm package: package.json at `d`, cases.ts at `sub` (default `d`) carrying `body`.

    `imports` is explicit because a fixture that imports what it does not use is not a smaller fixture —
    it is a DIFFERENT one, and `gen_netclass` deliberately pulls only `node:net` so its cases cannot
    accidentally answer for `Fs` or `Exec`."""
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, "package.json"), "w") as f:
        f.write(f'{{"name":"{name}","version":"0.0.0"}}\n')
    dest_dir = sub if sub is not None else d
    os.makedirs(dest_dir, exist_ok=True)
    with open(os.path.join(dest_dir, "cases.ts"), "w") as f:
        f.write(f"// GENERATED by {gen} -- do not edit.\n")
        f.write(imports + "\n")
        f.write(body + "\n")
