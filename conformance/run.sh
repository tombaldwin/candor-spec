...
echo "PART 48 — the file set: what a report says about code it never opened (SPEC §2 ⟨0.29⟩, verdict ⟨0.30⟩)"
# ENGINES: rust java ts swift
...
if [ "$P48_OK" = 0 ]; then
  echo "  -> MATCH — every engine ASKED reports the effect in the file it did not judge, makes the verdict"
  echo "     INCOMPLETE (exit 2) without folding it into \`functions\` or \`violations\` ⟨0.30⟩, says nothing"
  echo "     under a policy denying a different effect, and omits the key entirely when no policy was set"
else
  echo "  -> DIVERGE — see FAIL lines"; rc=1
fi
...
# ─────────────────────────────────────────────────────────────────────────────────────────────────
# PART 53 — THE PEEK IS A PRODUCER READING THE POLICY, SO §3.1 BINDS IT (SPEC §2 ⟨0.29⟩)      [TIER 1]
#
# §2 already says it — "Over a policy the engine REFUSES, the key is ABSENT for the reason §3.1 withholds
# `ok`". The clause shipped WITH the rung and three of the four engines did not implement it: over
# `deny Net` beside a typo'd effect token, candor-java withheld the key and candor-rust, candor-ts and
# candor-swift published it at exit 2. A MUST can exist in the spec and in one implementation.
#
# THE HARM IS THE KEY, NOT THE FINDING. `outOfScope: []` beside a refusal reads "a policy was configured,
# I looked at what it denies, and there is nothing" — a look taken against rules no route would honour,
# searching the parser's SALVAGE of an unhonourable file. That is the silent rewriting the refusal exists
# to prevent, one layer down from where it was prevented.
#
# THREE SHAPES: A refused (key ABSENT), B honoured-and-dirty (key PRESENT with the finding), C
# honoured-and-clean (key PRESENT and EMPTY). B stops an engine passing A by never publishing the key —
# maximally cautious, and it deletes the feature. C stops it collapsing asked-and-clear ⟨0.27⟩ into
# cannot-answer ⟨0.26⟩, which is the overload the rung exists to remove.
# ─────────────────────────────────────────────────────────────────────────────────────────────────
echo
P53_OK=0
RP="$W/refused53"; mkdir -p "$RP"
p53() { # $1 engine ; $2..$7 rc/report triples for shapes A B C
  python3 "$HERE/refused_peek_check.py" "$@" || P53_OK=1
}
echo "[53] THE PEEK IS A PRODUCER READING THE POLICY  (SPEC §2 ⟨0.29⟩ — a refused policy leaves the key ABSENT)"
printf 'deny Net\ndeny Nonsense\n' > "$RP/refused.pol"   # a typo'd effect token: every route refuses
printf 'deny Net\n' > "$RP/stands.pol"
printf 'deny Db\n'  > "$RP/other.pol"
if [ -x "$SCAN" ]; then
  mkdir -p "$RP/rs/src" "$RP/rs/tests"
  printf '[package]\nname="ref53"\nversion="0.0.0"\nedition="2021"\n' > "$RP/rs/Cargo.toml"
  printf 'pub fn lib_fn() -> i32 { 1 }\n' > "$RP/rs/src/lib.rs"
  printf 'pub fn t() { let _ = std::net::TcpStream::connect("1.2.3.4:80"); }\n' > "$RP/rs/tests/h.rs"
...
mkdir -p "$RP/java/src" "$RP/java/stage/lib"
printf 'package app;\npublic class M { public static int m() { return 1; } }\n' > "$RP/java/src/M.java"
printf 'package lib;\npublic class L { public static void reach() throws Exception { new java.net.Socket("1.2.3.4", 80).close(); } }\n' > "$RP/java/src/L.java"
javac -d "$RP/java/classes" "$RP/java/src/M.java" 2>/dev/null
...
# ── PART 54 — THE ⟨0.30⟩ VERDICT REACHES BOTH ROUTES IDENTICALLY (SPEC §2 ⟨0.30⟩ / §3.1)   [TIER 1]
# PART 48 drives the SCAN route. This drives BOTH, because §3.1's byte-equality is the acceptance test and
# only one of the two routes can peek: `gate --report` holds a document, not a tree. That asymmetry killed
# the `net-partner` attempt; ⟨0.30⟩ survives it because `outOfScope` rides the REPORT. See
# peek_route_equality_check.py for what each arm rules out, and why the absent-key control and the
# asked-and-clear control are NOT the same control.
#
...
if [ -n "$TS_PRESENT" ]; then
  ( cd "$TS_DIR" && node scan.mjs "$FS/ts/tsconfig.json" --out "$FS/ts/t54" --policy "$FS/ts/exec.pol" \
...
java -jar "$JAR" "$FS/java" --json "$FS/java/j54.json" --policy "$FS/java/exec.pol" \
...
echo "PART 54 — the ⟨0.30⟩ verdict reaches both routes identically (SPEC §2 ⟨0.30⟩ / §3.1)"
if [ "$P54_OK" = 0 ]; then
  echo "  -> MATCH — every engine ASKED reaches exit 2 from BOTH \`scan --policy\` and \`gate --report\`,"
  echo "     with byte-equal verdict documents carrying ok:false + incomplete + the findings and NO"
  echo "     violations; an ABSENT key stays exit 0, and so does an asked-and-clear peek"
else
  echo "  -> DIVERGE — see FAIL lines"; rc=1
fi
...
if [ -x "$SCAN" ]; then
  ...
  python3 - "$NPD" <<'PY57RS' || P57_OK=1
import glob, json, os, sys
base = sys.argv[1]
bad = 0
def rep(arm):
    c = [f for f in glob.glob(os.path.join(base, f"rs_{arm}", ".candor", "report*.json"))
...
if not (isinstance(np, dict) and ".candor/config" in str(np.get("config"))
    ...
    bad += 1
    print(f"  rust   A NAMED     report carries {np!r} — the config that moved the classification and the "
          f"host it declared must both be named")
sg = os.path.join(base, "rs_used", "scan.gate.json"); rg = os.path.join(base, "rs_used", "rep.gate.json")
if os.path.exists(sg) and os.path.exists(rg):
    if open(sg, "rb").read() != open(rg, "rb").read():
        bad += 1
        print("  rust   B ROUTES    `scan --policy` and `gate --report` verdicts are NOT byte-equal")
else:
    bad += 1
    print("  rust   B ROUTES    one of the two verdict documents was not written — cannot compare")
