...
P4G2_OK=0
printf '%s' '{ "meta": {"version":"t","toolchain":"stable","spec":"0.12"}, "packages": ["com.a.x","com.a.y"], "functions": [ {"fn":"settings::Settings::load","inferred":["Fs"],"calls":["io::write_file"]}, {"fn":"io::write_file","loc":"src/io.rs:3","inferred":["Fs"],"direct":["Fs"]} ] }' > "$W/plural/r.demo.scan.json"
plural_hdr rust "$W/plural/r" "$QUERY" || P4G2_OK=1
printf '%s' '{ "candor": {"version":"t","spec":"0.12"}, "packages": ["com.a.x","com.a.y"], "functions": [ {"fn":"com.a.x.Settings.load","inferred":["Fs"],"calls":["com.a.y.Disk.writeFile"]}, {"fn":"com.a.y.Disk.writeFile","loc":"Disk.java:3","inferred":["Fs"],"direct":["Fs"]} ] }' > "$W/plural/j.jvm.json"
plural_hdr java "$W/plural/j.jvm.json" java -jar "$JAR" || P4G2_OK=1
if [ -n "$TS_PRESENT" ]; then
  printf '%s' '{ "candor": {"version":"t","spec":"0.12"}, "packages": ["com.a.x","com.a.y"], "functions": [ {"fn":"com_a.Settings.load","inferred":["Fs"],"calls":["com_a.io.writeFile"]}, {"fn":"com_a.io.writeFile","loc":"src/io.ts:3","inferred":["Fs"],"direct":["Fs"]} ] }' > "$W/plural/t.JS.json"
  plural_hdr ts "$W/plural/t" node "$TS_DIR/query.mjs" || P4G2_OK=1
fi
if [ -n "$SW_PRESENT" ]; then
  printf '%s' '{ "candor": {"version":"t","spec":"0.12"}, "packages": ["com.a.x","com.a.y"], "functions": [ {"fn":"Settings.load","inferred":["Fs"],"calls":["Disk.writeFile"]}, {"fn":"Disk.writeFile","loc":"Sources/Disk.swift:3","inferred":["Fs"],"direct":["Fs"]} ] }' > "$W/plural/s.Swift.json"
...
# ====================================================================================================
# PART 5b — GAINS ORIGIN differential (spec 0.12, §3.1): each `gains --json` byFunction entry    [TIER 2]
...
gow() { # write one engine's fixture pair
  printf '{ "candor": {"version":"t","spec":"0.12"}, "functions": [ {"fn":"%s","inferred":["Fs"],"direct":["Fs"]} ] }' "$3" > "$5"
  printf '{ "%s": ["%s"], "%s": [] }' "$2" "$3" "$3" > "$6"
  printf '{ "candor": {"version":"t","spec":"0.12"}, "functions": [ {"fn":"%s","inferred":["Net"],"direct":["Net"]}, {"fn":"%s","inferred":["Fs"],"direct":["Fs"]}, {"fn":"%s","inferred":["Net"],"direct":["Net"]} ] }' "$2" "$3" "$4" > "$7"
}
...
# ====================================================================================================
# PART 12d — GATE AUTO-DISCLOSURE differential (spec 0.12 — candor-scan/java/ts/swift 0.12.0):   [TIER 2]
