...
sys.path.insert(0, HERE)  # so this module is importable from elsewhere, not only run as a script

import gen_differential as gd  # noqa: E402 -- the shared fixture-tree writers (one owner)
...
def _rust_tree(d, case):
    gd.write_rust_tree(d, "netclass", "gen_netclass.py", case["rust"].format())
    _write_config(d, case)


def _java_tree(d, case):
    gd.write_java_tree(d, "gen_netclass.py", case["java"].format())
    _write_config(d, case)


def _ts_tree(d, case):
    # netclass pulls ONLY node:net on purpose — see write_ts_tree's `imports` note.
    gd.write_ts_tree(d, "netclass", "gen_netclass.py", case["ts"].format(),
                     imports='import * as netm from "node:net";\n')
    _write_config(d, case)
