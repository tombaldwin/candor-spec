...
# A property nobody has seen FAIL is a property nobody has tested. `--fault` corrupts the a2recv arm's
# FIXTURE — it writes the a3handle body (a use-verb on an already-opened handle, which every engine
# correctly declines to mark) into the a2recv cell — so the arm that must FAIL now cannot, and the
# verdict must go red.
#
# THE FAULT IS AT THE FIXTURE, NOT AT THE COMPARISON, and that is the whole point. The registry's other
# four-way differentials sit in UNCOVERED because inverting an expected value only proves the comparison
# can subtract; it says nothing about whether the row's INPUT ever reached the engine. This one proves
# exactly that: if a2recv still reported red with the handle body in its tree, the cell would be reading
# something other than the fixture it names — which is the standing
# "prove the fixture reaches the code" failure, in the instrument rather than in a fix.
...
def render(ws):
    ...
    for arm, _rc, _why, skip in ARMS:
        cell = f"{EFF['id']}_{arm}"
        src = "a3handle" if (FAULT and arm == "a2recv") else arm
        if "rust" not in skip:
...
def main():
    ...
    if FAULT:
        print("PROBE: CANDOR_PROBE_FAULT is set — the a2recv cell is being written with the a3handle "
              "body (a use-verb on an already-opened handle). The arm that MUST fail now cannot, and "
              "this run's verdict MUST go red. A clean run must never print this line.")
    render(ws)
    available = []
