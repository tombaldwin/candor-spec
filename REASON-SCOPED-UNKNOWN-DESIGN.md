...
> classes on the fn, on an AS-EFF-006 Unknown denial) ships four-way; and the **config `unknown-alias` key**
> (§4 — a user-defined `.candor/config` `unknown-alias <name> = <class,…>` referenced explicitly as
> `Unknown[<name>]`; multi-value; discovered by walking up from the policy/scan target; a config alias may
