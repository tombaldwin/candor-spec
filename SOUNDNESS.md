...
| R52 | rust-scan | **var-clone rebind loses the pointee type**: `let a = Arc::new(Svc); let b = a.clone(); b.run()` reads pure — `a.clone()` is (correctly) the pure `Arc::clone` and NOT charged, but `b` gets no type (`ctor_type` doesn't consult `vars` for the clone receiver), so `b.run()` drops. R51 closed the direct `let a = Arc::new(..); a.run()` form; this is the clone-rebind sibling. Safe pre-designed fix: at the `let` binding, if `init` is `<expr>.clone()` and untyped, carry `resolve_recv_type(<expr>)` to the binding (clone is type-preserving → sound; the clone call is still not charged, so no fabrication). Deferred: low incidence (3–4 clone-rebinds per large crate, only a fraction method-called on a local pointee), and it touches `visit_local`'s hot typing path + has an uncertain spawn-move-closure interaction. | SILENT (open) | low/niche | honesty-first under-report; the anti-fabrication clone guard ([[test]] `..._but_not_clone`) must stay intact — carry the TYPE, never charge the clone |
