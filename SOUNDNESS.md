...
| effect mechanisms that die at the SCAN BOUNDARY | 2026-07-25 | **rust (OPEN)**, other engines unswept | **FALSE ALL-CLEAR ON A GATE.** Code candor analyses soundly in ONE crate loses the effect entirely when split across a crate boundary with the dep report chained: `format!("{}", e)` on a dep type whose `Display::fmt` reads the clock gives `describe -> ['Clock']` combined and **PURE** chained — and `deny Clock` **exits 1 combined, 0 chained** | five root causes ranked: the join fires only on crate-QUALIFIED paths (desugared `Display::fmt`/`Drop::drop` emit none, though the dep report holds the right entry); no return types travel, so a factory-bound receiver is untyped; `interfaceUnion` is shipped-but-OFF in the default `--deps` path; `trait_decls` is local-only; coverage is crate-granular so one resolved call clears the blind flag for every shape. Also: the sweep's first framing ("chaining deletes the disclosure") was CORRECTED — the miss exists with and without chaining. SOUNDNESS-VEIN-crossing-the-scan-boundary.md |
