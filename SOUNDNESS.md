...
| R17 | java (also the jsoup streaming-parser pattern) | I/O via an ABSTRACT `java.io` stream (`Reader`/`InputStream`/`Writer`/`OutputStream`) whose concrete impl candor can't pin reads PURE, not Unknown — e.g. `void parse(Reader r){ r.read(); }`. **Globally SOUND in the common case:** the concrete (FileReader / socket stream) is created in analyzed code, so the effect is attributed AT THE CREATION SITE — VERIFIED 2026-06-20: jsoup dogfood Net/Fs land at `connect`/`parse(File)`; the dynamic-oracle corpus `AbstractReaderParse`'s real Fs lands at `main`'s FileReader ctor. The genuine GLOBAL silent miss is NARROW: only when the concrete stream is created ENTIRELY outside analyzed code (a framework hands an entry point a `Reader`/`InputStream` over a file/socket, zero in-scope creation). Surfaced + allowlisted by the dynamic oracle (`soundness/dynamic/corpus.sh`). | SILENT per-method (globally sound except external-stream→entry-point) | low | WHY pure-not-Unknown = PRECISION: disclosing Unknown on every abstract-stream read floods (every `copy(Reader)` helper → Unknown). FIX OPTION (Tom's call — a precision tradeoff): provenance-gated, ENTRY-POINT-scoped Unknown — disclose Unknown only when an abstract-stream read traces to a rooted entry point's OWN param with no in-scope concrete; closes the framework-injected-stream case without flooding internal helpers. |
