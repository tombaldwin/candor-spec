...
| ~~R46~~ | rust-scan | dispatch long-tail: nested `Vec<Option<Box<dyn>>>` / `Option<Vec<Box<dyn>>>`, tuple-destructured dyn factory return, `Default::default()` turbofish. | ~~SILENT~~ MOSTLY CLOSED | v.low | **FIXED 2026-07-19 (`d2a58d2`)**: `elem_trait_leaves` peel steps compose (trait_leaves-or-elem_trait_leaves) + a `trait_vars` carry through the next unwrap → both nesting orders dispatch; the turbofish case already worked. RESIDUAL: only the tuple-destructured dyn-factory-return (`let (d,_)=make_pair()`) — needs a trait-typed tuple index, rare |
