...
| R30 | rust-scan | **trait DEFAULT method via an empty impl read silent-pure**: `trait Logger { fn log(&self){effect} }` + `impl Logger for S {}` + `s.log()` — the default body IS collected (`Logger::log` unit, carries the effect), but a concrete receiver `s: S` with no own `log` doesn't fall back to its implemented traits' defaults. Common Rust idiom. Found 2026-07-10 (autonomous cross-engine sweep). | SILENT | low-med | build a type→implemented-traits index at impl collection; in the method-call resolver, when `S::method` is absent, retry `<trait>::method` for each trait S implements that has a default |
...
2. **Oracle coverage** = # real crates × effects under dynamic ground truth (§3 #1). *2026-06-18: grew 7→11
   ...
   diversity.*
3. **Open SILENT residuals** (§5) = count by severity. *As of 2026-07-10: **9 open (R2–R8, R28, R30),
   all low/v.low/low-med; 0 med+** (R24–R27, R29 opened + FIXED same day; R28 conditional-conformance-on-stdlib open, niche). Everything opened since the baseline (R13, R14/R16, R17–R21) was driven to CLOSED — see the
