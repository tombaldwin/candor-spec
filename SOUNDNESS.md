...
| Provided io::Write/io::Read → local required-method impl (R32, direct-method sibling of R14) | 2026-07-18 | rust scan `445a1e0` + dylint lint | **SILENT-PURE** — `s.write_all()`/`r.read_to_end()` on a concrete local effectful `impl Write`/`Read` (std-provided body drives `self.write`/`read`, invisible); only the `write!`-macro edge was recovered | fixed BOTH front-ends (scan=Fs, lint=Fs*+Unknown); 0 over-fire A/B ~900 fns; conformance OK; regression + ui pins |
