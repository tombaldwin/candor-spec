...
| R16 | java | writer side of formatting — a custom effectful `Appendable`/`Writer` WRAPPED in a JDK `Formatter`/`PrintWriter` and driven by `format`/`printf`/`print` reads PURE (`new Formatter(out).format(..)` / `pw.printf(..)` → `out.append`/`writer.write` dropped). The 4th engine with the R14 class. candor-java HANDLES the arg-side (`append(Object)`→`toString` via `provFrames`/`ProvValue` + `reentryEdge`, CHA'd over the arg's DECLARED type) but the writer is the RECEIVER's wrapped sink (receiver→ctor-arg provenance), a deeper escape step. | SILENT | low | precise fix is FEASIBLE (extend the `reentryEdge`/provenance witness to trace a Formatter/PrintWriter receiver to its constructor sink arg → edge its `append`/`write`); CHA-blanket REJECTED by candor-java's precision design (the kotlinx-2160 blowup). Low: the custom-effectful-sink-via-JDK-facade idiom is rare. Repro 2026-06-18. |
...
4. **Find-rate** = cardinal sins found per fresh adversarial round. *2026-06-18: 6 seam-class rounds each found
   ...
   java), FIXED in 3, java tracked (R16, low). Convergence: 17 rounds, ~14 finds, 13 fixed + 1 tracked.*
