...
| Provided Write/Read method → local required-method impl (R32, direct-method sibling of R14/R16) | 2026-07-18 | rust scan+lint `445a1e0`; java `453cbe9` | **SILENT-PURE** — a std/JDK-provided Write/Read method drives the receiver's required override, invisibly: rust `s.write_all()`/`r.read_to_end()` on a concrete local impl; java `w.write(String)`/`r.read(char[])` on a Writer/Reader subclass. Only the format-macro/facade edge was recovered | fixed rust (scan=effect, lint=effect+Unknown) + java (reentry keyed on the RECEIVER's io type); 0 over-fire A/B (~900 rust + ~12k java fns) + a genuine jsoup Clock recovery; conformance OK; regressions + ui pins |
