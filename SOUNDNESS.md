...
| initializer edge into an unanalyzed dependency | 2026-07-25 | java + ts (**OPEN**) | **SILENT-PURE** — a module whose own top level is pure imports a dep whose top level is not; importing runs it, so the initializer transitively performs the effect and candor reports `(∅,∅)`. Found on real code (proper-lockfile → graceful-fs.js:35 `process.env.NODE_DEBUG`; write-file-atomic → Rand) | **RECORDED, NOT REPAIRED.** Concept is right in the reference engine — with the dep INSIDE the scanned set candor-java gives `app.App.<clinit> { Env* }` one hop via `dep.Dep.<clinit>`; outside it, dropped silently. Blanket disclosure MEASURED AND REJECTED (60–100% of modules across 6 packages, candor-ts itself 83%). Resolution is DETERMINATION not disclosure: chain the dep report (`deps`) or scan dependency initializers only. swift N/A (lazy globals), rust N/A (no top-level code). SOUNDNESS-VEIN-initializer-edge.md + fixtures |
