...
- **Conformance PART 10: the ambiguity fixture is re-pointed to a genuine ambiguity, and the part gains
  ...
  PICK, which is why nothing caught this before. **These two rows WERE red against candor-rust `cc05b8c` when this
  entry was written, deliberately** — the clause landed before the port. **They are GREEN as of
  candor-rust `7dfe710`, which carries SOUNDNESS R222's `by_leaf` dedup; the dedup is no longer
  withheld.** That engine commit is on `main` and ships in this release, and a reader of the sentence
  as first written would have believed the suite was knowingly red and the fix held back. The DIVERGE
  text says so, so a red PART 10 is not
